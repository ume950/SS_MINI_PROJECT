...
void save_faculty_db() {
    int fd = open("db/faculty.db", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0) {
        perror("open");
        exit(1);
    }
...
void save_user_db() {
    int fd = open("db/users.db", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0) {
        perror("open");
        exit(1);
    }
...
void save_courses_db() {
    int fd = open("db/courses.db", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0) {
        perror("open");
        exit(1);
    }
...
int create_course(char *name, int credits, char *instructor, int max_seats) {
    int course_db_fd = open("db/courses.db", O_RDWR | O_CREAT, 0644);
    if (course_db_fd < 0) {
        perror("open");
        exit(1);
    }
